...
class EngineConfig:
    ...
    def _load_score(self, song_name):
        ...
        try:
            with open(FILENAME_SCORE_DATA, 'r', encoding='utf-8') as file:
                ...
                print(f'self.score_data: {self.score_data}')
                for song in self.score_data:
                    print(f'=====\ndata:{song}, song_name:{song_name}, self.leaderboard: {self.leaderboard}\n====')
                    if song['song'] == song_name:
                        self.song_index = song_index
                        self.leaderboard = song['leaderboard']
                        print(f'self.leaderboard: {self.leaderboard}')
    ...
    def _update_score_json(self):
        with open('assets/score/score.json', 'w') as file:
            self.score_data[self.song_index]['leaderboard'] = self.leaderboard
            json.dump(self.score_data, file, indent=2)
